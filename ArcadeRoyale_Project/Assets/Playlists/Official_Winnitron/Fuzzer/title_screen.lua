...
function title_screen:update()
  ...
  if self.time > 120 then
    for i = 1, #self.player_inputs do
      if (self.player_inputs[i].acceleration > 0.7 or
          self.player_inputs[i].start) and
...
function title_screen:postdraw()
  local height = 60
  local hud_ratio = 4/3 -- text hardcoded for this ratio
  local screen_ratio = dokidoki.kernel.get_ratio()

  gl.glMatrixMode(gl.GL_PROJECTION)
  gl.glLoadIdentity()
  gl.glOrtho(0, screen_ratio*height, 0, height, 100, -100)
  ...
  gl.glVertex2d(0, 0)
  gl.glVertex2d(screen_ratio*height, 0)
  gl.glVertex2d(screen_ratio*height, height)
  gl.glVertex2d(0, height)
  gl.glEnd()
  gl.glBlendFunc(gl.GL_SRC_ALPHA, gl.GL_ONE)

  gl.glTranslated((screen_ratio-hud_ratio)*height/2, height-line_height/2, 0)
