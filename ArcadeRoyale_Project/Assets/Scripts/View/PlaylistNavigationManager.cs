...
public class PlaylistNavigationManager : MonoBehaviour {
    ...
    void Update() {
        ...
        if (!loading && GM.worldState == GM.WorldState.Launcher) {
	            ...
	            if (playlistManagerList.Count > 1 && playerActions.Right.WasPressed) {
                ...
                else if (playlistManagerList.Count > 1 && playerActions.Left.WasPressed) {
            ...
            if (!moving) {
                ...
                // Launch game
                if (playerActions.Enter.WasPressed) {
                    ...
                    Debug.Log("current playlist: " +  playlistManagerList[selectedPlaylistIndex].name);
